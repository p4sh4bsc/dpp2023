...
data = list()
data_length = 0
ser = serial.Serial("/dev/serial/by-path/platform-fd500000.pcie-pci-0000:01:00.0-usb-0:1.2:1.0-port0", 38400, timeout=1)
...
def main():
    ...
    while not rospy.is_shutdown():
        ...
        for i in range(img_len // 512):
            meta = get_meta()
            ser.write(b"BGN" + frame[512*i:512*(i+1)] + meta + b"END")
            read_answer()
        ost = img_len % 512
        if ost != 0:
            meta = get_meta()
            ser.write(b"BGN" + frame[-ost:] + meta + b"END")
            read_answer()
        meta = get_meta()
        ser.write(b"BGN" + b"FRAME" + meta + b"END")
        read_answer() 
